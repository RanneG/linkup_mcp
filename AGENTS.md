...
- **linkup_mcp launcher handoff:** root **`Stitch.bat`**, **`Stitch-Desktop.bat`**, **`stitch_gui.py`**, and **`scripts/Start-Stitch*.ps1`** were removed from this repo. Canonical double-click launchers now live in **stitch-app**; keep this repo available for `stitch_rag_bridge.py` on `8765`.
- **Native desktop (Tauri dev):** Double-click **`Stitch-Desktop.bat`** in the **stitch-app** repo (or run `npm run dev` there). It uses this repo's Flask bridge for `/api/*`; start `stitch_rag_bridge.py` here if the stitch-app launcher does not start it for you. Requires **Rust + Tauri prerequisites** ([Tauri docs](https://v2.tauri.app/start/prerequisites/)).
- **Single-window (recommended daily driver):** double-click **`Stitch.bat`** in the **stitch-app** repo. It uses this repo for bridge/backend capabilities when needed; see **stitch-app** `docs/RUNNING.md` for the current bundled flow.
- **Python install profiles:** **`uv sync`** (or `pip install -e .`) = MCP + RAG only. **`uv sync --extra stitch-bridge`** = HTTP bridge, face, Google APIs, server-side STT. CI runs contract tests on the default profile and imports + face storage tests with **`stitch-bridge`**.
